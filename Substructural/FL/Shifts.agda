...
lemma1-2 {j} {R} iFL e (inr (inl rp)) {a} {b} =
  ByRule (iFL' (L· {U = []} {V = []} {a = j a} {b = j b})) (d5 ∷ᵃ []ᵃ)
  where
  iFL' : FLRules ⊆R (R ∪R CommRules)
  iFL' rr = inl (iFL rr)
...
lemma1-2 {j} {R} iFL e (inr (inr bp)) {a} {b} =
...
lemma1-3 {j} {R} iFL e pn {a} {b} =
  ...
  step-shift· = lift-⊆R embed (lemma1-2 iFL e pn)
...
lemma1-4
  ...
  → Shift· j (L⟨ R ⟩)
  → BiNucleus j (L⟨ R ⟩)
lemma1-4 n s = nucleus→biNucleus n
...
lemma1-5 {j} {R} iFL bn =
  ...
  l⊸j {U} {V} {W} {a} {b} {c} dU dWV =
    ...
    dWV' : Deriv R (W ++ j b ∷ V) (j c)
    dWV' = ljL {U = W} {V = V} {a = b} {b = c} dWV

    core : Deriv R ((W ++ U) ++ ((j a ⊸ j b) ∷ V)) (j c)
    core =
      ByRule
  ...
  l›j {U} {V} {W} {a} {b} {c} dU dWV =
    ...
    core : Deriv R (W ++ (j b › j a) ∷ (U ++ V)) (j c)
    core =
      ByRule
        (iFL (L› {U = U} {V = V} {W = W} {a = j a} {b = j b} {c = j c}))
        (dU ∷ᵃ dWV' ∷ᵃ []ᵃ)
...
lemma2
  ...
  → (G⟨ j , R ⟩ ⊆ L⟨ ShiftCoreExt j R ⟩)
    × (L⟨ ShiftCoreExt j R ⟩ ⊆ G⟨ j , R ⟩)
lemma2 lj surv s = g→ext lj surv , ext→g s
...
lemma2-from-base-shifts lj surv s =
  lemma2 lj surv (shiftCore-base→G s)
